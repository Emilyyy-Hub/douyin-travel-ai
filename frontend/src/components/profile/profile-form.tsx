...
type RequiredField = keyof Pick<
  UserProfile,
  "height" | "size" | "stylePreference" | "budget" | "photoGoal" | "shoePreference" | "skinExposure"
>;
      ...
      <Card className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <SelectField
            id="shoePreference"
            label="鞋子偏好"
            value={profile.shoePreference}
            error={errors.shoePreference}
          ...
          <SelectField
            id="skinExposure"
            label="露肤接受程度"
            value={profile.skinExposure}
            error={errors.skinExposure}
...
function SelectField({ id, label, value, options, error, onChange }: SelectFieldProps) {
  return (
    <div>
      <label className="text-sm font-semibold text-[#24211d]" htmlFor={id}>
        {label}
      </label>
      <select
        id={id}
...
function TextAreaField({ id, label, value, placeholder, error, onChange }: TextAreaFieldProps) {
  return (
    <div>
      ...
      <textarea
        id={id}
        className="focus-ring mt-2 min-h-24 w-full rounded-lg border border-[#d8cabc] bg-white px-3 py-3"
